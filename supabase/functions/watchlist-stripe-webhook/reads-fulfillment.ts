import { paidRead, settledCharge, READ_SLUG } from "../innerg-reads/rules.ts";
...
export async function fulfillRead(stripe: any, service: any, sessionId: string) {
  ...
  const {error} = await service.from("innerg_read_purchases").update({session_id:sessionId,status:"paid",paid_at:new Date().toISOString()}).eq("id",data.id);
