...
(async () => {
  ...
  for (const width of [1440,390]) {
    ...
    assert.equal(requestedSlug,'philly-money-moving');
    assert.equal(await page.locator('#article-nav').isVisible(),true);
    assert.equal(await page.locator('#read-index').isVisible(),false);
    assert.equal(await page.locator('#read-title').textContent(),'is philly actually making money ~ or does it just look like money is moving?');
    assert.equal(await page.locator('.art-concepts').isVisible(),false);
    assert.equal(await page.locator('[data-read-slug="philly-money-moving"]').getAttribute('aria-current'),'page');
    assert.equal(await page.locator('#support').isVisible(),true);
    assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),'latest article horizontal overflow');
    await page.screenshot({path:'/tmp/innerg-reads-003-'+width+'.png',fullPage:true}); await page.close();
    console.log(width+'px: article 003 title, archive state, public body and support placement passed');
  }
  for (const width of [1440,390]) {
    ...
    assert.equal(await page.locator('.read-list details').count(),3);
    assert.deepEqual(await page.locator('.read-list time').allTextContents(),['sep 12, 2026','sep 12, 2026','sep 11, 2026']);
    ...
    assert((await page.locator('.read-list-copy').first().textContent()).includes('read article 003'));
    await page.keyboard.press('Space');
