...
(async () => {
  ...
  for (const width of [1440,390]) {
    ...
    const articleSummary=page.locator('.read-list summary').first();
  ...
  for (const width of [1440,390]) {
    ...
    await page.goto(origin+'/reads/'); await page.locator('#full-read').waitFor({state:'visible'});
    assert.equal(requestedSlug,'pull-the-plug-on-intelligence');
    assert.equal(await page.locator('#collection-number').textContent(),'the personal reads collection / no. 002');
