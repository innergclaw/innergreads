...
(async () => {
  ...
  for (const width of [1440,390]) {
    ...
    assert.equal(await page.locator('.read-list details').count(),1);
    assert.equal(await page.locator('.read-list time').getAttribute('datetime'),'2026-09-11');
    assert.equal(await page.locator('.read-list time').textContent(),'sep 11, 2026');
    const articleSummary=page.locator('.read-list summary');
    await articleSummary.focus(); await page.keyboard.press('Enter');
    assert.equal(await page.locator('.read-list details').getAttribute('open'),'');
    assert((await page.locator('.read-list-copy').textContent()).includes('read article 001'));
    await page.keyboard.press('Space');
    assert.equal(await page.locator('.read-list details').getAttribute('open'),null);
