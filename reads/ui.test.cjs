...
(async () => {
  ...
  for (const width of [1440,390]) {
    ...
    await page.goto(origin+'/reads/?read=ya-hochu-zhenu'); await page.locator('#full-read').waitFor({state:'visible'});
    assert.equal(requestedSlug,'ya-hochu-zhenu');
    assert.equal(await page.locator('#article-nav').isVisible(),true);
    assert.equal(await page.locator('#read-index').isVisible(),false);
    assert.equal(await page.locator('#read-title').textContent(),'Я хочу жену');
    assert.equal(await page.locator('.art-concepts').isVisible(),false);
    assert.equal(await page.locator('[data-read-slug="ya-hochu-zhenu"]').getAttribute('aria-current'),'page');
    ...
    await page.screenshot({path:'/tmp/innerg-reads-005-'+width+'.png',fullPage:true}); await page.close();
    console.log(width+'px: article 005 title, archive state, public body and support placement passed');
  }
  for (const width of [1440,390]) {
    ...
    assert.equal(await page.locator('.read-list details').count(),5);
    assert.deepEqual(await page.locator('.read-list time').allTextContents(),['sep 19, 2026','sep 14, 2026','sep 12, 2026','sep 12, 2026','sep 11, 2026']);
    ...
    assert((await page.locator('.read-list-copy').first().textContent()).includes('read article 005'));
    await page.keyboard.press('Space');
